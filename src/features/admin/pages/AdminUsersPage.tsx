import { ArrowClockwise, MagnifyingGlass, Plus, SlidersHorizontal, Stethoscope, Tray, UsersThree, UserCheck, UserPlus } from '@phosphor-icons/react'
...
const usersPaginationLabels = {
  rows: 'Rows',
  previous: 'Previous',
  next: 'Next',
  nav: 'User list pagination',
...
export function AdminUsersPage() {
  ...
  return <div className="admin-page admin-users-page">
    ...
    {section === 'users' && <section className="admin-card admin-users-list-card">
      ...
      {listState !== 'error' && page.totalItems > 0 && <TablePagination
        page={page.page}
        pageSize={page.pageSize}
        totalItems={page.totalItems}
        totalPages={page.totalPages}
        pageSizes={adminUserPageSizes}
        busy={listState === 'loading'}
        itemNoun="users"
        labels={usersPaginationLabels}
        onPageChange={(next) => setParameter('page', next > 1 ? String(next) : '')}
        onPageSizeChange={(next) => setParameter('pageSize', String(next))}
      />}
    </section>}
