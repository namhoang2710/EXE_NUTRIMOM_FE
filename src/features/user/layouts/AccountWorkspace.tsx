...
export function AccountWorkspace() {
  ...
  useEffect(() => {
    function onEscape(event: KeyboardEvent) { if (event.key === 'Escape') setMobileOpen(false) }
  ...
  return <div className="nm-account-workspace">
    <button className="nm-account-mobile-toggle" type="button" onClick={() => setMobileOpen(true)} aria-expanded={mobileOpen} aria-controls="nm-account-sidebar"><List size={20} /> Danh mục tài khoản</button>
    <aside className={`nm-account-sidebar${mobileOpen ? ' is-open' : ''}`} id="nm-account-sidebar" aria-label="Danh mục tài khoản">
      <div className="nm-account-sidebar-top"><div className="nm-account-sidebar-avatar" aria-hidden="true">{initial}</div><div><strong>{name}</strong><span>Không gian cá nhân</span></div><button className="nm-account-sidebar-close" type="button" aria-label="Đóng danh mục" onClick={() => setMobileOpen(false)}><X size={20} /></button></div>
