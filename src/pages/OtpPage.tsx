  ...
  async function sendOtp() {
    ...
    try {
      const response = await requestOtp({
        phone: phone.trim(),
        purpose,
        accepted_terms: isRegister ? accepted : undefined,
        device_id: getDeviceId(),
      })
      setChallenge(response)
      setCooldown(response.resend_after)
      setCode('')
      setMessage(`Mã OTP đã được gửi tới ${response.masked_phone}.`)
    } catch (requestError) {
    ...
    >
      {!challenge ? (
        ...
        <form className="auth-form otp-form" onSubmit={handleVerify} noValidate>
          ...
          {challenge.debug_code && import.meta.env.DEV && (
            <div className="debug-code">
              <div>
                <span>Mã thử từ backend</span>
                <strong>{challenge.debug_code}</strong>
              </div>
              <button type="button" onClick={() => setCode(challenge.debug_code || '')}>Điền mã</button>
            </div>
          )}
