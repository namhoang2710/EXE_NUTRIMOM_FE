...
export function HealthPage() {
  ...
  return <AppShell>
    ...
    <section className="app-card calculator-card"><div className="section-title-row"><div><p className="card-kicker">Cập nhật tuổi thai</p><h2>Tính thử trước khi lưu</h2></div><Calculator size={30} weight="duotone" /></div><div className="method-tabs">{([{ value: 'LAST_MENSTRUAL_PERIOD', label: 'Kỳ kinh cuối' }, { value: 'CONCEPTION_DATE', label: 'Ngày thụ thai' }, { value: 'MANUAL', label: 'Nhập thủ công' }] as const).map((item) => <button key={item.value} type="button" className={method === item.value ? 'active' : ''} onClick={() => { setMethod(item.value); setPreview(null); setError('') }}>{item.label}</button>)}</div>{method === 'MANUAL' ? <div className="inline-fields"><label className="field-group"><span>Tuần thai</span><div className="input-shell"><input type="number" min="0" max="42" value={week} onChange={(event) => setWeek(event.target.value)} /></div></label><label className="field-group"><span>Ngày thai</span><div className="input-shell"><input type="number" min="0" max="6" value={day} onChange={(event) => setDay(event.target.value)} /></div></label></div> : <DateField label={method === 'CONCEPTION_DATE' ? 'Ngày thụ thai ước tính' : 'Ngày đầu kỳ kinh cuối'} value={date} onChange={(event) => setDate(event.target.value)} hint="Nhập theo định dạng YYYY-MM-DD." />}<p className="disclaimer">{calculationNote(method)}</p><button className="secondary-button" type="button" onClick={() => void calculate()} disabled={busy}>Tính kết quả</button>{preview && <div className="calculation-result"><strong>Dự sinh {formatDate(preview.estimated_due_date)}</strong><span>Tuần {preview.gestational_week}, ngày {preview.gestational_day} · Tam cá nguyệt {preview.trimester}</span><button className="primary-button" type="button" onClick={() => void applyPreview()} disabled={busy}><FloppyDisk size={20} />Dùng kết quả này</button></div>}</section>
  </AppShell>
}
